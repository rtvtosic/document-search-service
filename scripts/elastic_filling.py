...
from config import sync_client, sync_db_engine
...
def fill_elastic():
    ...
    if sync_client.indices.exists(index="documents"):
        sync_client.indices.delete(index="documents")
    ...
    sync_client.indices.create(index="documents", mappings=mappings)
    ...
    try:
        with Session(bind=sync_db_engine) as db:
            ...
            success, failed = bulk(sync_client, documents_dict)
